    ...
    (trackLoaded [track]
      (let [info (.getInfo track)]
        (if (.getPlayingTrack player)
          (do
            (channel-commands/send-message-to-channel! (str "Adding to queue: " (formatters/title-from-info info)))
      ...
      (let [tracks (.getTracks playlist)
        ...
        (if (.isSearchResult playlist)
          (do
            (if (.getPlayingTrack player)
              (do
                (channel-commands/send-message-to-channel! (str "Adding to the queue: " (formatters/title-from-info info)))
